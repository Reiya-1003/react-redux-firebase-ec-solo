...
function Kakunin(){
  ...
  const [user,setUser] = useSelector(cartSelector);
  // const [user, setUser] = useState(Choice)


    
    const [hyouzipiza,setHyouzi] = useState(user.Items)
...
　　　
     ...
     const orderCheck = () => {
        ...
        if (user.tell === "") { Validation.push("※電話番号を入力してください※") }
      ...
      tell:user.tell,
        ...
        User.push(cartinUser)
        User[0].Items=[chosepiza]
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <input type="text"
                       onChange={(e) => {
                        setUser({ ...user, tell: e.target.value })
                      }}
                      ref={(element) => input = element}></input>
                    </td>
                  </tr>
