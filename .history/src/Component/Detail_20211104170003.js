...
import { getCartItem, setCartItem } from '../actions/actions'
...
console.log(user)
    ...
    const [inpiza,setInpiza]=useState(Choice.Items) //choiceに入れるためのステート
    console.log(inpiza)
...
　　 
    ...
    return<ul  key={index}><div>◯{topp.name}
           ...
           onChange={toppCell} 
            ...
            ></input></p>}
            {inpiza.size === "L" && <p>Lsize Topping{topp.priceL}円<input type="checkbox" value={topp.name}
           id={topp.name}
            onChange={toppCell}
    ...
    const cartIn =()=>{
        ...
        if (user.uid) {
            if (Choice && Choice.Items) {
                ...
                const carts = Choice.Items.slice()
                //.slice()で()のなか（配列のインデックス番号）をコピーする
                //何も書かない場合全てコピーする。この場合Choice.Itemsをcartsにコピー
                carts.push(cartinPiza)
                Choice.Items = carts
                console.log(Choice)
                firebase.firestore()
            ...
            const cart2 = Choice ? Choice.slice() : {}
            // console.log(cart2)
            cart2.Items = Choice.Items === undefined ? [] : Choice.Items.slice()
            // console.log(cart2)
            cart2.Items.push(cartinPiza)
            // console.log(cart2)
            dispatch(setCartItem(cart2))
            handle('/cart')
        }
        }
